...
class LotkaVottera(BaseDynamics):
    def simulate(self, G, L, init=None, gr=None, cap=None, inter=None,
        ...
        abundances in an ecosystem. Species $i$'s abundance change per time is
        $\frac{d X_i}{d t} = r_i * X_i * \left(1 - \frac{X_i}{K_i} + \sum_{j \neq i} W_{ij} \frac{X_j}{K_i}\right)$
        where $r_i$ and $K_i$ are the growth rate and the carrying capacity of
        ...
        init (np.ndarray): Length-$N$ 1D array of nodes' initial condition. If
                           not specified an initial condition is unifromly
                           generated from 0 to the nodes' carrying capacity.

        gr (np.ndarray): Length-$N$ 1D array of nodes' growth rate. If not
                         specified, default to 1 for all nodes.

        cap (np.ndarray): Length-$N$ 1D array of nodes' carrying capacity. If
                          not specified, default to 1 for all nodes.

        inter (np.ndarray): $N \times N$ array of interaction weights between
                            nodes. If not specified, default to a zero-diagonal
                            matrix whose [i, j] entry is $frac{sign(j - i)}{N - 1}$.

        dt (float or np.ndarray): Sizes of time steps when simulating the
                                  continuous-time dynamics.

        stochastic (bool): Whether to simulate the stochastic or deterministic
                           dynamics.

        pertb (np.ndarray): Length-$N$ 1D array of perturbation magnitude of
                            nodes' growth. If not specified, default to 0.01
                            for all nodes.
        ...
        Notes
        -----
        1. The deterministic dynamics is simulated through the forth-order
           Runge-Kutta method, and the stochastic one is simulated through
           mutliplicative noise with the Euler-Maruyama method.

        2. The ground-truth network, time steps and the time series can be
        ...
        if not stochastic:
            ...
            for t in range(L-1):
                if integrator.succesful():
                    ...
                    message += 'Change sizes of time steps or the paramters.'
